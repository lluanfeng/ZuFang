...
<DIV class=lefter>
<H1>${current.title}</H1>
<DIV class=subinfo><c:formatDate value="${date}" pattern="yyyy-MM-dd HH:mm:ss"></c:formatDate></DIV>
<DIV class=houseinfo>
<P>户　　型：<SPAN>${current.tname}</SPAN></P>
<P>面　　积：<SPAN>${current.floorage}m<SUP>2</SUP></SPAN></P>
<P>位　　置：<SPAN>${current.dname}${current.sname}</SPAN></P>
<P>联系方式：<SPAN>${current.contact}</SPAN></P></DIV></DIV>
<DIV class=side>
